...
function _i2c_mb_eeprom_init {
    ...
    if [ "${action}" == "new" ] && \
        ...
        echo "24c32 0x55" > ${PATH_I801_DEVICE}/new_device
    elif [ "${action}" == "delete" ] && \
           [ -L ${PATH_SYS_I2C_DEVICES}/${NUM_I801_DEVICE}-0055 ]; then
...
#Init PSU EEPROM and PMBUS
function _i2c_psu_eeprom_pmbus_init {
    ...
    if [ "${action}" == "new" ] && \
        ...
        ## PS0 EEPROM
        echo "spd 0x50" > ${PATH_SYS_I2C_DEVICES}/i2c-${NUM_FRU_MUX_CHAN1_DEVICE}/new_device
        ## PS1 EEPROM
        echo "spd 0x50" > ${PATH_SYS_I2C_DEVICES}/i2c-${NUM_FRU_MUX_CHAN0_DEVICE}/new_device

	## PS0 PMBUS
        echo "pmbus 0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${NUM_FRU_MUX_CHAN1_DEVICE}/new_device
        ## PS1 EEPROM
        echo "pmbus 0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${NUM_FRU_MUX_CHAN0_DEVICE}/new_device
    elif [ "${action}" == "delete" ] && \
           [ -L ${PATH_SYS_I2C_DEVICES}/${NUM_FRU_MUX_CHAN1_DEVICE}-0050 ] || \
           [ -L ${PATH_SYS_I2C_DEVICES}/${NUM_FRU_MUX_CHAN0_DEVICE}-0050 ]; then
        ## PS0 EEPROM
        ...
        ## PS0 PMBUS
        echo "0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${NUM_FRU_MUX_CHAN1_DEVICE}/delete_device
        ## PS1 PMBUS
        echo "0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${NUM_FRU_MUX_CHAN0_DEVICE}/delete_device
    fi
    echo "DONE"
}
...
function _i2c_sfp_eeprom_init {
	...
	echo e1-33 > /sys/bus/i2c/devices/45-0050/port_name
	echo e2-34 > /sys/bus/i2c/devices/46-0050/port_name
    elif [ "${action}" == "delete" ] && \
         [ -L ${PATH_SYS_I2C_DEVICES}/${NUM_SFP1_DEVICE}-0050 ] && \
         [ -L ${PATH_SYS_I2C_DEVICES}/${NUM_SFP2_DEVICE}-0050 ]; then
...
function _main {
    ...
    if [ "${EXEC_FUNC}" == "help" ]; then
        ...
        _i2c_psu_eeprom_pmbus_init ${MB_EEPROM_ACTION}
