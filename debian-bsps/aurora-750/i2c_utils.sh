...
function _i2c_deinit {
    echo "i2c deinit..."
    for mod in coretemp jc42 w83795 netberg_aurora_750_i2c_cpld at24 gpio-pca953x i2c_mux_pca954x i2c_i801;
    do
        _util_rmmod $mod
    done
    echo "Done"
...
function _i2c_cpld_init {
    echo "CPLD init..."
    _util_rmmod netberg_aurora_750_i2c_cpld
    modprobe netberg_aurora_750_i2c_cpld
    ...
    do
        ...
        if ! [ -L ${dev_path} ]; then
            echo "netberg_cpld${i} ${I2C_ADDR_CPLD}" > ${!cpld_path}/new_device
        else
...
function _i2c_io_exp_init {
    ...
    _util_i2cset -y -r ${I2C_BUS_MAIN} ${I2C_ADDR_MUX_9555_2} ${REG_PORT1_DIR} 0xF3
...
function _i2c_mb_eeprom_init {
    ...
    if [ "${action}" == "new" ] && \
        ...
        echo "24c32 ${I2C_ADDR_MB_EEPROM}" > ${PATH_SYS_I2C_DEVICES}/i2c-${I2C_BUS_MB_EEPROM}/new_device
    elif [ "${action}" == "delete" ] && \
        [ -L ${PATH_SYS_I2C_DEVICES}/${I2C_BUS_MB_EEPROM}-$(printf "%04x" $I2C_ADDR_MB_EEPROM) ]; then
...
function _i2c_psu_init {
    ...
    echo "spd ${I2C_ADDR_PSU1_EEPROM}" > ${PATH_SYS_I2C_DEVICES}/i2c-${I2C_BUS_PSU1_EEPROM}/new_device
    echo "spd ${I2C_ADDR_PSU2_EEPROM}" > ${PATH_SYS_I2C_DEVICES}/i2c-${I2C_BUS_PSU2_EEPROM}/new_device

    echo "pmbus 0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${I2C_BUS_PSU1_EEPROM}/new_device
    echo "pmbus 0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${I2C_BUS_PSU2_EEPROM}/new_device
}
...
function _i2c_psu_deinit {
    ...
    echo "0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${I2C_BUS_PSU1_EEPROM}/delete_device
    echo "0x58" > ${PATH_SYS_I2C_DEVICES}/i2c-${I2C_BUS_PSU2_EEPROM}/delete_device
...
function _i2c_psu_eeprom_get {
    ...
    eeprom_psu1="${PATH_SYSFS_PSU1}/eeprom"
    cat ${eeprom_psu1} | hexdump -C

    eeprom_psu2="${PATH_SYSFS_PSU2}/eeprom"
...
function _i2c_psu_status {
    ...
    psu1PwGood=0
    psu1PwGood=0
    if test -f /sys/bus/i2c/devices/18-0058/hwmon/hwmon*/in1_input; then
        psu1Exist=1
        if [ `cat /sys/bus/i2c/devices/18-0058/hwmon/hwmon*/in1_input` -gt 0 ]; then
            psu1PwGood=1
        fi
    fi

    if test -f /sys/bus/i2c/devices/17-0058/hwmon/hwmon*/in1_input; then
        psu2Exist=1
        if [ `cat /sys/bus/i2c/devices/17-0058/hwmon/hwmon*/in1_input` -gt 0 ]; then
            psu1PwGood=1
        fi
    fi
